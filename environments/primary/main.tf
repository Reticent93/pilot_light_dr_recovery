...
module "asg" {
  ...
  iam_instance_profile       = data.terraform_remote_state.global.outputs.instance_profile_name
  ...
  ami_id                     = ""
  eip_allocation_id          = module.vpc.ec2_eip_allocation_id
}
...
resource "aws_lambda_permission" "allow_sns_invocation" {
  statement_id  = "AllowExecutionFromSNSTopicPrimary"
